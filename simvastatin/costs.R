...
costs <- function(arrivals)
{
  ...
  stats <- do.call(rbind, lapply(split(arrivals, arrivals$name), FUN=function(x)
  {
    ...
    results <- c(sum(x$discounted_cost), (sum(life$discounted_time) - sum(x$disutility))/365)
    names(results) <- c("Discounted Cost", "QALY")
    results
  }))
  ...
  list(stats=stats,boot=bo,disc_cost95=dc95,qaly95=qaly95)
}