...
# Note : This approach is more readible than Arena
...
asps <- function(deathRisk)
{
  create_trajectory("ASPS") %>%
  ...
  branch(function(attrs) sample(1:2, 1, prob=twoway(attrs, "strokeRisk")),
         ...
         create_trajectory("Stroke Event") %>%
           ...
           release("strokes") %>%
           branch(function() sample(1:2, 1, prob=c(1-deathRisk, deathRisk)),
                  merge=c(T, F),
  ...
  timeout(1) %>%
  rollback(amount=5,times=4) # Do it 4 more times, for 5 years total
}

shinyLife <- function(deathRisk)
{
  traj <- asps(deathRisk)
  sapply(1:100, FUN=function(x) {
    ...
    arrivals$resource <- factor(arrivals$resource, c("strokes", "deaths"))
    xtabs( ~ resource, arrivals)
  })
}
